...
const unitColumns = `id, unit_number, samsara_vehicle_id, company_id, fuel_level_percent, mpg, monday_item_id, created_at, updated_at`
...
// Samsara collector sets SamsaraVehicleID/FuelLevelPercent/MPG, the DB/Sheets
// collector sets CompanyID, the Monday publisher sets MondayItemID) and
// existing values are preserved when nil.
...
func UpsertUnit(ctx context.Context, pool *pgxpool.Pool, in UnitUpsert) (*Unit, error) {
	row := pool.QueryRow(ctx, `
		INSERT INTO units (unit_number, samsara_vehicle_id, company_id, fuel_level_percent, mpg, monday_item_id)
		VALUES ($1, $2, $3, $4, $5, $6)
		...
		in.UnitNumber, in.SamsaraVehicleID, in.CompanyID, in.FuelLevelPercent, in.MPG, in.MondayItemID)
...
func scanUnit(row rowScanner) (*Unit, error) {
	var u Unit
	if err := row.Scan(&u.ID, &u.UnitNumber, &u.SamsaraVehicleID, &u.CompanyID, &u.FuelLevelPercent, &u.MPG, &u.MondayItemID, &u.CreatedAt, &u.UpdatedAt); err != nil {
		return nil, err
	}
	return &u, nil
}
